        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[1000]">
          <div className="bg-white p-6 rounded-lg shadow-lg w-80 text-center z-[1050]">
            ...
            <div className="flex justify-center space-x-4">
              <button className={`px-4 py-2 rounded-md text-white ${isLoggingOut ? "bg-gray-400 cursor-not-allowed" : "bg-red-500 hover:bg-red-700"}`} onClick={handleLogout} disabled={isLoggingOut}>
                {isLoggingOut ? (
              ...
              <button className="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400" onClick={() => setIsLogoutConfirmOpen(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
